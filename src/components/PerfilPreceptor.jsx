...
const PerfilPreceptor = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    const url = `${backurl.replace(/\/$/, '')}/preceptores/${userId}`;
    fetch(url, {
  ...
  const handleSave = () => {
    ...
    fetch(`${backurl.replace(/\/$/, '')}/preceptores/${userId}`, {
      method: 'PUT',
      headers: {
  ...
  return (
    <div style={{ padding: '20px' }}>
      <h2>Perfil de Preceptor: {user.nombre}</h2>
      {editMode ? (
        <div>
          <input
            type="text"
...
export default PerfilPreceptor;